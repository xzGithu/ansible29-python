...
"""
通过ansible API动态生成ansible资产信息 最后调用这个类来生成主机信息。
"""
...
class PlaybookCallbackModule(CallbackBase):
    ...
    def v2_runner_on_ok(self, result, *args, **kwargs):
        if result._host.get_name() in self.host_ok.keys():
            try:
                ...
                self.host_ok[result._host.get_name()].append(result.task_name+' '+result._result["stderr"][:200])
            except:
                self.host_ok[result._host.get_name()].append(result.task_name)
        else:
            try:
                self.host_ok[result._host.get_name()] = [result.task_name+' '+result._result["stderr"][:200]]
    ...
    def v2_runner_on_failed(self, result, *args, **kwargs):
        if result._host.get_name() in self.host_failed.keys():
            self.host_failed[result._host.get_name()].append(result.task_name+' '+result._result["msg"][:200])
        else:
            self.host_failed[result._host.get_name()] = [result.task_name+' '+result._result["msg"][:200]]
        #print(result._result)
...
class MyInventory:
    def __init__(self, hostsresource, sources=None):
        ...
        """
        sources代替hosts文件，它可以是一个列表里面包含多个文件路径且文件真实存在，在单纯的执行ad-hoc的时候这里的
...
class AnsibleAPI:
    ...
    def get_play_result(self):
        #results_raw = {'skipped': {}, 'failed': {}, 'ok': {}, "status": {}, 'unreachable': {}, "changed": {}, "password": {}}
        results_raw = {'failed': {}, 'ok': {}, 'unreachable': {}, 'password': {}}
        for host, result in self.callback.host_ok.items():
        ...
        results_raw['password'] = self.callback.host_password
        print(json.dumps(results_raw, indent=4))

def main():
    temphosts_list = [{"ip": "192.168.181.129", "port": "22", "username": "root", "password": None}]
